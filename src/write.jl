...
function writexyz(io::IO, cell::Cell, comment="")
...
function writecif(io::IO, cell::Cell)
    a, b, c, α, β, γ = basisvectors(Lattice(cell))
    block = CifBlock()
    ...
    return block["_atom_site_fract_z"] = zf
end
...
function writevasp(io::IO, cell::Cell; scaling_factor=1, cartesian=false, comment="")
    println(io, comment)
    println(io, scaling_factor)
    lattice = Lattice(cell) / scaling_factor
    for vector in basisvectors(lattice)
